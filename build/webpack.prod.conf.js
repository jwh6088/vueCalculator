...
//引入uitls工具配置文件用来处理css类文件的loader
const utils = require('./utils')
//引入webpack使用webpack内置插件
const webpack = require('webpack')
//引入config目录下的index.js配置文件，主要用来定义了生产和开发环境的相关基础配置
const config = require('../config')
//引入webpack的merger插件，主要用来处理配置对象合并的，可以将一个大的配置对象拆分成几个小的，合并，相同的项将覆盖
const merge = require('webpack-merge')
//引入webpack.base.conf.js配置文件，用来处理不同类型文件的loader
...
还要安装 cssnano 这是一个css编译器 npm install --save-dev cssnano 这个vue-cli脚手架并没有使用cssnano，但是这个插件的官方说要安装cssnano
...
if (config.build.productionGzip) {
  ...
  // vue-cli默认将这个功能禁用掉，理由是Surge 和 Netlify 静态主机默认帮你把上传的文件gzip了
